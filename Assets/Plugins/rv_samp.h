...
namespace pf {
    namespace rvsamp {
        ...
        //! すべての乱数サンプラータイプの基底クラス。主な利点はシードを設定することです。
        /**
         * @class rvsamp_vase
         * @brief 全ての乱数サンプラーはこのクラスを継承する必要があります。
         * @tparam dim 各乱数ベクトルサンプルの次元。
         */
        class rvsamp_base {
        public:

            /**
             * @brief デフォルトコンストラクタ。唯一のオブション。クロックでシードを設定します。
             */
            inline rvsamp_base() :
        ...
        //! 一変量正規分布からのサンプリングを行うクラス。
        /**
         * @class UnivNormSampler
         * @brief 一変量正規分布からサンプルを生成します。
         */
        ...
        public:
            ...
            /**
             * @brief デフォルトコンストラクタには標準正規乱数生成のために設定されます。
             */
            UnivNormSampler();

            /**
             * @brief ユーザーは平均と標準偏差の両方を指定する必要があります。
             * @param mu サンプリング分布の平均を表すfloat_t。
             * @param sigma サンプリング分布の標準偏差を表すfloat_t (>0)。
             */
            UnivNormSampler(float_t mu, float_t sigma);

            /**
             * @brief サンプラーの平均を設定します。
             * @param mu 望ましい平均。
             */
            void setMean(float_t mu);

            /**
             * @brief サンプラーの標準偏差を設定します。
             * @param sigma 望ましい標準偏差。
             */
