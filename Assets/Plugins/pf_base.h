...
namespace pf {
    namespace bases {
        ...
        * @author t
        * @file pf_base.h
        * @brief 共変量を持たないすべての非Rao-Blackwellized粒子フィルタはこのクラスを継承します。
        * このクラスを継承することで、粒子フィルタの基本的な機能を実装することができます。
        * @tparam float_t 浮動小数点数の型（例：double, floatなど）
        * @tparam dimobs 観測の次元
        * @tparam dimstate 状態の次元
        * @tparam dimcov 共変量の次元
        */
        template<typename float_t, size_t dimobs, size_t dimstate>
        class pf_base {
        public:
            /* using宣言は、型エイリアスを定義するために使用されます。
            これにより、コードの可読性が向上し、型の変更が容易になります。*/
            ...
            /* 各観測ベクトルの次元（テンプレートパラメータへの間接アクセスを許可） */
            static constexpr unsigned int dim_obs = dimobs;

            /* 状態ベクトルの次元（テンプレートパラメータへの間接アクセスを許可） */
            static constexpr unsigned int dim_state = dimstate;

            /**
             * @brief フィルタリング関数。各観測データを使ってフィルタリングを行います。
             * @param data 最新の観測データ
             * @param fs 各時点で期待値を計算するための関数のベクター
             */
            virtual void filter(const obs_sized_vec& data, const func_vec& fs = func_vec()) = 0;

            /**
             * @brief 条件付き対数尤度を取得するためのゲッターメソッド
             * @return log p(y_t| y_{1:t-1}) の近似値
             */
